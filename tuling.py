...
def picdeal(msg):
    ...
    if fileSize == tempSize:
        ...
        try:
            itchat.send_image('f:/wxPic/'+str(totalPicNumber) +
                              '.gif', toUserName=msg['FromUserName'])
        except:
...
def picture_reply(msg):
    ...
    try:
        ...
        picFromName = itchat.search_friends(
            userName=msg['FromUserName'])['NickName']
        dic[msg['FromUserName']] = picFromName
    try:
        picToName = dic[msg['ToUserName']]
    except:
        picToName = itchat.search_friends(
            userName=msg['ToUserName'])['NickName']
        dic[msg['ToUserName']] = picToName
    print('%s--->%s:<Pic>%s' %
          (picFromName, picToName, msg['FileName']))  # friend/me : xxx.Pic
    if picFromName != myName:
        try:
            ...
            if resend == 0:
                ...
                try:
                    itchat.send_image('f:/wxPic/'+str(i)+'.gif',
                                      toUserName=msg['FromUserName'])
                    print('%s(Robot)--->%s:%s' %
                          (myName, picFromName, str(i)+'.gif'))
                except:
...
def recording_reply(msg):
    try:
        ...
        recFromName = itchat.search_friends(
            userName=msg['FromUserName'])['NickName']
        dic[msg['FromUserName']] = recFromName
    try:
        recToName = dic[msg['ToUserName']]
    except:
        recToName = itchat.search_friends(
            userName=msg['ToUserName'])['NickName']
        dic[msg['ToUserName']] = recToName
    print('%s--->%s:<Rec>%s' %
          (recFromName, recToName, msg['MsgId']))  # friend/me : xxxPic
...
def map_reply(msg):
    try:
        ...
        mapFromName = itchat.search_friends(
            userName=msg['FromUserName'])['NickName']
        dic[msg['FromUserName']] = mapFromName
    try:
        mapToName = dic[msg['ToUserName']]
    except:
        mapToName = itchat.search_friends(
            userName=msg['ToUserName'])['NickName']
